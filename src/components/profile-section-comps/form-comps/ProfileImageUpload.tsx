  ...
  return (
    <div className="w-full grid gap-4 p-5 bg-lightGrey rounded-xl smallTablet:gap-10 smallTablet:grid-cols-formColumns lgLaptop:gap-20">
      ...
      <div className="flex flex-col justify-start items-start gap-6 tablet:flex-row tablet:items-center">
        <div className={`text-center relative px-[38px] py-[60px] rounded-xl bg-purpleLight font-semibold inline-flex flex-col justify-center items-center overflow-hidden hover:cursor-pointer ${currentUpload ? "text-white" : "text-purple"}`}>
          {currentUpload && (
            <>
              <Image
                width={1024}
          ...
          <span className="relative flex flex-col justify-center items-center z-20">
            <UploadImageIcon colour={currentUpload ? "#ffffff" : "#633CFF"} /> + Upload Image
          </span>
          <input
